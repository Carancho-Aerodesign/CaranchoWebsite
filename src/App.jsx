...
import { Shield, Users, LogIn, LogOut, User, Lock, Twitter, Instagram, Facebook, Trophy, Star, Award, Menu, X, ChevronLeft, ChevronRight, Briefcase, Crown, UserCheck, Hash, GraduationCap, PlusCircle, Trash2, Edit, Save, LayoutDashboard, Image as ImageIcon, Link as LinkIcon, AlertCircle, CheckCircle, XCircle, UploadCloud, Settings, Building, ChevronDown, MapPin, Mail, Eye, EyeOff, DollarSign, FileDown, Circle, ClipboardList } from 'lucide-react';
...
const LoadingScreen = () => ( <div className="fixed inset-0 bg-white/70 backdrop-blur-sm flex items-center justify-center z-[100]"><div className="flex flex-col items-center"><img src="./logoWithLabelBright.svg" alt="Logo Carancho Aerodesign" className="h-16 animate-pulse" /></div></div> );
const Navbar = ({ currentPage, setCurrentPage, user, auth }) => { const [isMenuOpen, setIsMenuOpen] = useState(false); const handleNavClick = (page) => { setIsMenuOpen(false); setCurrentPage(page); }; const handleLogout = async () => { if(auth) { await signOut(auth); } handleNavClick('home'); }; return ( <nav className="bg-white/80 backdrop-blur-lg fixed top-0 left-0 right-0 z-40 border-b border-gray-200"><div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><div className="flex items-center justify-between h-20"><div className="flex-shrink-0 flex items-center cursor-pointer" onClick={() => handleNavClick('home')}><img className="h-10" src="./logoWithLabelBright.svg" alt="Logo Carancho Aerodesign" /></div><div className="hidden md:block"><div className="ml-10 flex items-baseline space-x-2"><NavItem onClick={() => handleNavClick('home')} active={currentPage === 'home'}>Início</NavItem><NavItem onClick={() => handleNavClick('about')} active={currentPage === 'about'}>Sobre Nós</NavItem><NavItem onClick={() => handleNavClick('projects')} active={currentPage === 'projects'}>Projetos</NavItem>{user && <NavItem onClick={() => handleNavClick('admin')} active={currentPage === 'admin'}>Painel Admin</NavItem>}</div></div><div className="flex items-center"><div className="hidden md:block">{user ? <div className="flex items-center gap-4"><span className="text-sm text-gray-600">Olá, {user.email.split('@')[0]}</span><LogoutButton onClick={handleLogout} /></div> : <LoginButton onClick={() => handleNavClick('login')} />}</div><div className="md:hidden"><button onClick={() => setIsMenuOpen(!isMenuOpen)} className="p-2 rounded-md text-gray-600 hover:text-gray-900 hover:bg-gray-100">{isMenuOpen ? <X size={24} /> : <Menu size={24} />}</button></div></div></div></div>{isMenuOpen && (<div className="md:hidden bg-white border-t border-gray-200"><div className="px-2 pt-2 pb-3 space-y-1 sm:px-3"><NavItemMobile onClick={() => handleNavClick('home')} active={currentPage === 'home'}>Início</NavItemMobile><NavItemMobile onClick={() => handleNavClick('about')} active={currentPage === 'about'}>Sobre Nós</NavItemMobile><NavItemMobile onClick={() => handleNavClick('projects')} active={currentPage === 'projects'}>Projetos</NavItemMobile>{user && <NavItemMobile onClick={() => handleNavClick('admin')} active={currentPage === 'admin'}>Painel Admin</NavItemMobile>}</div><div className="p-4 border-t border-gray-200">{user ? <div className="flex flex-col items-start gap-4"><span className="text-sm text-gray-600 px-3">Olá, {user.email.split('@')[0]}</span><LogoutButton onClick={handleLogout} fullWidth /></div> : <LoginButton onClick={() => handleNavClick('login')} fullWidth />}</div></div>)}</nav> );};
...
const NavItemMobile = ({ onClick, children, active }) => (<a href="#" onClick={(e) => { e.preventDefault(); onClick(); }} className={`block px-3 py-2 rounded-md text-base font-medium transition-colors duration-300 ${active ? 'bg-[#d4982c] text-white' : 'text-gray-600 hover:bg-gray-100 hover:bg-gray-900'}`}>{children}</a>);
    ...
    return (
        <div className="max-w-4xl mx-auto py-16 px-4 sm:px-6 lg:px-8 animate-fade-in-up">
            <div className="text-center mb-16">
                ...
                <p className="mt-2 max-w-2xl mx-auto text-lg text-gray-600">Uma viagem pela nossa história de inovação e engenharia.</p>
            </div>
                    ...
                    <div className="space-y-16">
                        {sortedProjects.map((project, index) => (
                            <div key={project.id} className="relative">
                                <div className="absolute left-1/2 -translate-x-1/2 -top-1 w-4 h-4 rounded-full bg-[#d4982c] border-4 border-gray-100 hidden md:block"></div>
                                <div className={`flex flex-col md:flex-row items-center gap-8 ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
                                    <div className="md:w-1/2">
                                        <img src={project.imageUrl || 'https://placehold.co/600x400/e0e0e0/888888?text=Projeto'} alt={project.title} className="w-full h-auto rounded-lg shadow-lg object-cover"/>
                                    </div>
...
        e.preventDefault();
        if (!auth) { setError('Serviço de autenticação indisponível. Tente novamente mais tarde.'); return; }
        setError('');
        try { await signInWithEmailAndPassword(auth, email, password); } catch (err) { setError('E-mail ou senha inválidos.'); console.error("Login Error:", err); }
    };
    return (
...
                    <div><button type="submit" disabled={!isAuthReady} className="w-full flex justify-center py-3 px-4 text-sm font-medium rounded-md text-white bg-[#d4982c] hover:bg-[#b58426] disabled:bg-gray-400 transition-all duration-300 transform hover:scale-105 shadow-lg">Entrar</button></div>
                </form>
                {!isAdminRegistered && (
                    <p className="mt-2 text-center text-sm text-gray-600">Não tem uma conta de administrador?{' '}<button onClick={() => setCurrentPage('register')} className="font-medium text-[#d4982c] hover:text-[#b58426]">Registe-se aqui</button></p>
                )}
            </div>
        </div>
    );
...
        } catch (err) { setError('Erro ao registar. Verifique o e-mail e a senha (mínimo 6 caracteres).'); console.error("Registration Error:", err); }
    };
    return (
        <div className="w-full h-full flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 animate-fade-in-up">
            <div className="max-w-md w-full space-y-8 bg-white backdrop-blur-sm p-10 rounded-2xl shadow-2xl border border-gray-200">
...
                    </div>
                    {error && <p className="text-red-500 text-sm text-center">{error}</p>}
                    <div><button type="submit" disabled={!isAuthReady} className="w-full flex justify-center py-3 px-4 text-sm font-medium rounded-md text-white bg-[#d4982c] hover:bg-[#b58426] disabled:bg-gray-400 transition-all duration-300 transform hover:scale-105 shadow-lg">Registar</button></div>
                </form>
                 <p className="mt-2 text-center text-sm text-gray-600">Já tem uma conta?{' '}<button onClick={() => setCurrentPage('login')} className="font-medium text-[#d4982c] hover:text-[#b58426]">Inicie a sessão</button></p>
            </div>
        </div>
    );
};
...
const AdminPage = ({ db, storage, teamHierarchy, sponsors, achievements, projects, siteSettings, setNotification, financials }) => {
...
    const [financialYear, setFinancialYear] = useState(new Date().getFullYear());
    const [projectForm, setProjectForm] = useState({ id: null, year: new Date().getFullYear(), title: '', description: '', imageUrl: '' });
...
            if (checked) { return { ...prev, generalRoles: [...currentValues, value] }; } else { return { ...prev, generalRoles: currentValues.filter(item => item !== value) }; }
        });
    };
...
        if (file) { setMemberImageFile(file); setMemberImagePreview(URL.createObjectURL(file)); }
    };
...
                newHierarchy.members.forEach(member => { if (member.id !== memberId && member.generalRoles?.includes('Capitão')) { member.generalRoles = member.generalRoles.filter(r => r !== 'Capitão'); } });
            }
...
        newHierarchy.members.forEach(member => { member.assignments = member.assignments.filter(a => a.department !== teamHierarchy.departments.find(d => d.id === deptId).name); });
...
    };
    ...
    const handleProjectSubmit = async (e) => {
        ...
        try {
            ...
            const projectData = { year: Number(projectForm.year), title: projectForm.title, description: projectForm.description, imageUrl: imageUrl, };
            if (editingProjectId) {
...
const SelectField = ({ name, value, onChange, Icon, children }) => (<div className="relative flex-grow"><span className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none z-10"><Icon className="h-5 w-5 text-gray-400" /></span><select name={name} value={value} onChange={onChange} className="appearance-none rounded-md block w-full pl-10 pr-8 py-3 border border-gray-300 bg-white text-gray-900 focus:outline-none focus:ring-[#d4982c] focus:border-[#d4982c] sm:text-sm">{children}</select><span className="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none"><ChevronDown className="h-5 w-5 text-gray-400" /></span></div>);
...
const YoutubeIcon = (props) => (<svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21.54 6.22a4.5 4.5 0 0 0-3.18-3.18C16.9 2.5 12 2.5 12 2.5s-4.9 0-6.36.54a4.5 4.5 0 0 0-3.18 3.18C2 7.68 2 12 2 12s0 4.32.54 5.78a4.5 4.5 0 0 0 3.18 3.18C7.1 21.5 12 21.5 12 21.5s4.9 0 6.36-.54a4.5 4.5 0 0 0 3.18-3.18C22 16.32 22 12 22 12s0-4.32-.46-5.78z" /><polygon points="9.5,7.5 15.5,12 9.5,16.5" /></svg>);
const Footer = () => (<footer className="bg-gray-900 text-gray-400"><div className="max-w-7xl mx-auto py-12 px-4 sm:px-6 lg:px-8"><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8"><div className="md:col-span-2 lg:col-span-1"><img className="h-12 mb-4" src="./logo.png" alt="Logo Carancho Aerodesign" /><p className="text-sm">Projetando o futuro da aviação, um voo de cada vez.</p></div><div><h3 className="text-lg font-semibold text-white uppercase tracking-wider">Contato</h3><div className="mt-4 space-y-4 text-sm"><div className="flex items-start gap-3"><MapPin size={18} className="text-[#d4982c] shrink-0 mt-1" /><span>Av. Roraima nº 1000, Cidade Universitária, Bairro Camobi, Santa Maria - RS, 97105-900</span></div><div className="flex items-center gap-3"><Mail size={18} className="text-[#d4982c] shrink-0" /><a href="mailto:carancho@ufsm.br" className="hover:text-[#d4982c] transition-colors">carancho@ufsm.br</a></div></div></div><div><h3 className="text-lg font-semibold text-white uppercase tracking-wider">Siga-nos</h3><div className="flex mt-4 space-x-6"><a href="https://www.linkedin.com/company/caranchoaerodesign" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-[#d4982c] transition-colors"><span className="sr-only">LinkedIn</span><LinkedinIcon /></a><a href="https://www.youtube.com/@CaranchoAer/" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-[#d4982c] transition-colors"><span className="sr-only">YouTube</span><YoutubeIcon /></a><a href="https://www.instagram.com/caranchoufsm" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-[#d4982c] transition-colors"><span className="sr-only">Instagram</span><Instagram /></a><a href="https://www.facebook.com/caranchoufsm" target="_blank" rel="noopener noreferrer" className="text-gray-400 hover:text-[#d4982c] transition-colors"><span className="sr-only">Facebook</span><Facebook /></a></div></div></div><div className="mt-12 border-t border-gray-700 pt-8 text-center"><p className="text-base text-gray-400">&copy; {new Date().getFullYear()} Carancho Aerodesign. Todos os direitos reservados.</p></div></div></footer>);
const SponsorsCarousel = ({ sponsors }) => {
    if (sponsors.length === 0) return null;
    const animationDuration = sponsors.length * 5;
    return (
        <section className="py-12 bg-gray-100">
            <div className="text-center mb-10 px-4"><h2 className="text-3xl md:text-4xl font-bold text-[#d4982c]">Nossos Parceiros</h2></div>
            <div className="w-full inline-flex flex-nowrap overflow-hidden group" style={{ maskImage: 'linear-gradient(to right, transparent 0, black 10%, black 90%, transparent 100%)' }}>
...
                    {[...sponsors, ...sponsors].map((sponsor, index) => (<div key={index} className="mx-8 flex-shrink-0"><img src={sponsor.logo} alt={sponsor.name} className="max-h-24 w-auto" /></div>))}
                </div>
            </div>
        </section>
    );
...
      case 'admin': 
        return user ? <AdminPage db={db} storage={storage} teamHierarchy={teamHierarchy} sponsors={sponsors} achievements={achievements} projects={projects} siteSettings={siteSettings} financials={financials} setNotification={setNotification} /> : <LoginPage auth={auth} isAuthReady={isAuthReady} setCurrentPage={setCurrentPage} setNotification={setNotification} isAdminRegistered={isAdminRegistered} />;
